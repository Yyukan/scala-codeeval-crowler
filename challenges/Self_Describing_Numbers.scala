...
object Main extends App {
  ...
  for (line <- lines) {
    println(isSelfDescribing(line))
  }

  def isSelfDescribing(line: String):Int = {
    val occurrences:Map[Int, Int] = line.foldLeft(Map[Int,Int]().withDefaultValue(0))((occurrences, number: Char) =>
      occurrences + (number.asDigit -> (occurrences(number.asDigit) + 1)))

    for (i <- 0 until line.length) {
      val digit = line(i).asDigit

      occurrences.get(i) match {
        case Some(value) => if (value != digit) return 0
        case None => if (digit != 0) return 0
      }
    }
    1
  }
